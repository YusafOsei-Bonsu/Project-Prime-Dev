...
\includegraphics[width=12cm]{/design/mobile/UI}
...
\includegraphics[width=12cm]{/design/mobile/architecture}
...
\includegraphics[width=12cm]{/design/mobile/usecase}
...
\hline
1 & Server connection & Connects to client upon request through HTTP GET/POST requests  & As expected & PASS \\
\hline
2 & Database connection & Establish and maintain connection with the MongoDB instance, send and receive data & As expected & PASS \\
\hline
3 & Return files for display & After desktop client opens, all database files will display themselves in a list & As expected & PASS \\
\hline
4 & Upload file into database & After a file has been selected and the \textit{Submit} button has been pressed, the file will appear in the database & As expected & PASS \\
\hline
5 & Remove database file & After the \textit{Delete} button has been pressed, the server will remove the corresponding database file & As expected & PASS \\
\hline
6 & Return files for download & After the \textit{Download} button has been pressed, the server will return the corresponding file to the  host's file manager to be saved & As expected & PASS \\
\hline
\end{tabular}
...
2 & Database connection & Establish and maintain connection with the MongoDB instance, send and receive data & As expected & PASS \\
\hline
3 & Return files for display &  After opening the mobile app, all database files will display themselves in a list & As expected & PASS \\
\hline
4 & Upload files into database & After a file is selected from the Android internal storage, it will be uploaded into the database by the server & As expected & PASS \\
\hline
5 & Remove database files & After pressing the \textit{Delete} button next to a file, the corresponding document will be removed from the database by the server & As expected & PASS \\
\hline
6 & Return files for download & After pressing the \textit{Download} button, the server returns a file into the phone's internal storage & As expected & PASS \\
\hline
\end{tabular}
...
\begin{center}
\includegraphics[width=3.5cm]{/fragments/mobile/FileList}
\end{center}
...
\begin{center}
\includegraphics[width=3.5cm]{/fragments/mobile/NavBar}
\end{center}
...
\begin{center}
\includegraphics[width=3.5cm]{/fragments/mobile/FileOptions}
\end{center}
...
\includegraphics[width=10cm]{/fragments/mobile/RequestB}
