...
\subsection{\underline{Desktop Client}}
The desktop client was developed using the Electron framework where HTML, Javascript and CSS were primarily involved in the client's development. Electron provides pre-written HTML code (Electron, n.d.) to create a basic desktop app; this pre-written code was used as the foundation for the development of the desktop client. Afterwards, the client's front-end was modified using JS, HTML and CSS to reflect the project designs. Furthermore on the desktop client's front-end, Bootstrap CSS files and JS scripts were also included to style certain elements (e.g. headers) of the client's UI, along with CSS files written by the desktop subgroup. 

Below is a set of black-box test cases. Each test case executes an external functionality of the desktop client to determine if the corresponding MoSCoW requirements were achieved.
