...
As mentioned before the server application is created using Node.js. The server waits for requests from other clients and responds to them, thus providing a service upon their request. Thus the server application is responsible for handling requests, responding to them appropriately and for establishing the database connection.  We decided using Node.js to build the server as it allows us to write the application in JavaScript. Besides that, Node.js provides us the ability to use JavaScript on both frontend and backend.
 
To store and retrieve files we decided to use MongoDb. Unlike a relational database where data is stored in rows and columns, MongoDb stores JSON documents in collections with dynamic schemas.

Furthermore, we have used Express, which is a flexible Node application framework. It allowed us to set up the middleware and helps us to manage everything from routes to handling requests.
